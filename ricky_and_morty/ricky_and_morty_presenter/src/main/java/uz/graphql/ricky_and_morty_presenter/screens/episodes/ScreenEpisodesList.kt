package uz.graphql.ricky_and_morty_presenter.screens.episodes

import androidx.compose.foundation.clickable
import androidx.compose.foundation.layout.Column
import androidx.compose.material.Text
import androidx.compose.runtime.Composable
import androidx.compose.ui.Modifier
import androidx.navigation.NavHostController
import androidx.navigation.compose.rememberNavController
import uz.graphql.ricky_and_morty_presenter.navigation.ScreenRoutes

/**
 * Created by Saidmurod Turdiyev (S.M.T) on 2/22/2023 3:39 PM for Ricky And Morty.
 */
@Composable
fun ScreenEpisodesList(
    navController: NavHostController = rememberNavController()
) {
//    Column {
//        Text(text = "Episode", modifier = Modifier.clickable {
//            navController.navigate(route = ScreenRoutes.Episode.EpisodeDetailsScreen.route)
//        })
//        Text(text = "Episodes", modifier = Modifier.clickable {
//            navController.navigate(route = ScreenRoutes.Episode.EpisodesListDetailsScreen.route)
//        })
//    }
}